...
int do_eat(t_philo *philo)
{
    ...
    while (*(philo->data->death) == 0 && get_time() - philo->last_eat < (unsigned long)philo->data->time_to_eat)
    {}
    pthread_mutex_unlock(philo->l_fork);
    pthread_mutex_unlock(philo->r_fork);
    return (0);
...
void *run_monitor(void *philo_tmp)
{
    ...
    philo = philo_tmp;
    while (*(philo->data->death) == 0 && philo->my_death == 0 && philo->data->must_eat_cnt != philo->eat_count)
    {
        if (philo->data->must_eat_cnt != philo->eat_count && get_time() - philo->last_eat > (unsigned long)philo->data->time_to_die)
        {
            *(philo->data->death) += 1;
            philo->my_death = 1;
        }
